...
export function CreateCampaignPage() {
  ...
  function showTestNotice(message: string, variant: "success" | "error") {
  ...
  async function handleSendTest() {
    ...
    try {
      ...
      showTestNotice(result.message || `Test message queued from ${senderLabel} to ${testPhoneNumber.trim()}.`, "success");
    } catch (error) {
      showTestNotice(error instanceof Error ? error.message : "Unable to send test message.", "error");
    } finally {
            ...
            <section className="rounded-3xl border border-border bg-card p-5 shadow-soft">
              ...
              <label className="block">
                ...
                <Select
                  ...
                  disabled={!shouldFetchOrganizationData || isLoadingSavedTemplates}
                >
                  <option value="">
...
function formatSenderLabel(account: { name: string; phone_number?: string | null; phone_number_normalized?: string | null; status: string; live_status_error?: string | null; live_connection_status?: string | null }) {
...
function isSenderConnected(account: { status: string; live_connected?: boolean | null }) {
...
function formatSenderStatus(account: { status: string; live_status_error?: string | null; live_connection_status?: string | null }) {
